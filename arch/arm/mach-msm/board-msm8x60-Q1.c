...
unsigned int get_hw_rev(void)
{
    ...
    const hw_rev_mapping hw_rev_map[] = {
        ...
        //{ .real_hw_rev = 0x07, .hw_rev = 0x06 },
        { .real_hw_rev = 0x08, .hw_rev = 0x07 },
        //{ .real_hw_rev = 0x07, .hw_rev = 0x08 },
        //{ .real_hw_rev = 0x07, .hw_rev = 0x09 },
        //{ .real_hw_rev = 0x07, .hw_rev = 0x0A },
...
//    RPM_VREG_INIT_LDO_PF(PM8058_L8,  0, 1, 0, 1800000, 3050000, LDO300HMIN, RPM_VREG_PIN_CTRL_NONE, RPM_VREG_PIN_FN_SLEEP_B),
    RPM_VREG_INIT_LDO_PF(PM8058_L8,  0, 1, 0, 1800000, 3300000, LDO300HMIN, RPM_VREG_PIN_CTRL_NONE, RPM_VREG_PIN_FN_SLEEP_B),
...
//	RPM_VREG_INIT_LDO_PF(PM8058_L20, 0, 1, 0, 1800000, 1800000, LDO150HMIN, RPM_VREG_PIN_CTRL_NONE, RPM_VREG_PIN_FN_SLEEP_B),
	RPM_VREG_INIT_LDO_PF(PM8058_L20, 0, 1, 0, 1800000, 3050000, LDO150HMIN, RPM_VREG_PIN_CTRL_NONE, RPM_VREG_PIN_FN_SLEEP_B),
...
int p6_wacom_vdd_enable(int onoff) // to turn off the wacom in sleep
{
	int rc;
...
EXPORT_SYMBOL(p6_wacom_vdd_enable);
...
static int  p6_wacom_settle_down_befor_poweron(void)
{
	int ret;
	int rc;
...
static int __init p6_wacom_init(void)
{
	...
	p6_wacom_settle_down_befor_poweron();
#else
	p6_wacom_init_hw();
#endif
...
#if defined(CONFIG_SAMSUNG_EARJACK) 
...
static void __init msm8x60_map_io(void)
{
	...
	sec_getlog_supply_meminfo(0x8000000, 0x40000000, 0x38000000, 0x48000000);
#endif
}
...
*/
	...
	if (get_hw_rev() >= 0x5) {
		if(vsensor_2p85_magnetic == NULL) {
			vsensor_2p85_magnetic = regulator_get(&rpm_vreg_device[RPM_VREG_ID_PM8058_L20].dev, "8058_l20");
			if (IS_ERR(vsensor_2p85_magnetic))
			return -1;
	
			ret = regulator_set_voltage(vsensor_2p85_magnetic, 3000000, 3000000);
			if (ret)
...
#ifdef CONFIG_MSM_BUS_SCALING
#if 0 // defined (CONFIG_FB_MSM_MIPI_S6E8AA0_HD720_PANEL) // kyNam_110920_ Fix Problem : sometimes flicking while Camera-preview 
 ...
 */
extern void set_lcd_esd_forced_ignore(int);
static int mipi_dsi_panel_power(int on)
{
	int flag_on = !!on;
	...
	{ 
		...
		if (on) {
		set_lcd_esd_forced_ignore(false);		
			/* set ldo0 to HPM */
		...
		set_lcd_esd_forced_ignore(true);      		
...
#if defined (CONFIG_USA_MODEL_SGH_I717)
/* Define added NC GPIO port, since rev04 */

#define GPIO_34	  34

static uint32_t I717_nc_port_config[] = {
...
static void __init gpio_init_nc_port(void)
{
	...
	if (HWREV >= 0x04) {
		config_gpio_table(I717_nc_port_config, ARRAY_SIZE(I717_nc_port_config));
		printk("[SGH_I717] %s: since rev04 (this rev=0x%x), Initialize NC port to IN/PD\n", __func__, HWREV);
#if 0
		printk("[SGH_I717] %s: NC Port list: ", __func__);
		for (n = 0; n < ARRAY_SIZE(I717_nc_port_config); n++) {
			gpio = GPIO_PIN(I717_nc_port_config[n]);
			printk("%d, ", gpio);
		}
		printk("\n");
#endif
	} else {
		printk("[SGH_I717] %s: Do not initialize NC port, this rev below rev04(this rev=0x%x)\n", __func__, HWREV);
	}
}
#endif
