...
#if defined (CONFIG_SEC_DEBUG)
#define Q6ASM_OPEN_FAIL_PANIC_TIMEOUT
#endif
...
#ifdef Q6ASM_OPEN_FAIL_PANIC_TIMEOUT
static int open_fail_panic_timeout_cnt = 30;
#endif

static void dump_qdss_reg(void)
{
	void __iomem *reg_base;

	reg_base = ioremap(0x28000000, SZ_256);
	pr_info("0x28000040=0x%08X\n", readl(reg_base + 0x40));
	pr_info("0x28000044=0x%08X\n", readl(reg_base + 0x44));
	iounmap(reg_base);

	reg_base = ioremap(0x28800000, SZ_256);
	pr_info("0x28800000=0x%08X\n", readl(reg_base + 0x00));
	pr_info("0x28800004=0x%08X\n", readl(reg_base + 0x04));
	pr_info("0x28800008=0x%08X\n", readl(reg_base + 0x08));
	pr_info("0x2880001C=0x%08X\n", readl(reg_base + 0x1C));
	pr_info("0x28800020=0x%08X\n", readl(reg_base + 0x20));
	iounmap(reg_base);

	reg_base = ioremap(0x28887000, SZ_256);
	pr_info("0x2888700C=0x%08X\n", readl(reg_base + 0x0C));
	pr_info("0x28887010=0x%08X\n", readl(reg_base + 0x10));
	pr_info("0x28887014=0x%08X\n", readl(reg_base + 0x14));
	pr_info("0x28887018=0x%08X\n", readl(reg_base + 0x18));
	pr_info("0x2888701C=0x%08X\n", readl(reg_base + 0x1C));
	pr_info("0x28887020=0x%08X\n", readl(reg_base + 0x20));
	iounmap(reg_base);

	reg_base = ioremap(0x2888a000, SZ_256);
	pr_info("0x288A0000=0x%08X\n", readl(reg_base + 0x00));
	pr_info("0x288A0014=0x%08X\n", readl(reg_base + 0x14));
	pr_info("0x288A001C=0x%08X\n", readl(reg_base + 0x1C));
	pr_info("0x288A0020=0x%08X\n", readl(reg_base + 0x20));
	iounmap(reg_base);

	reg_base = ioremap(0x288b0000, SZ_256);
	
	pr_info("0x288B0000=0x%08X\n", readl(reg_base + 0x00));
	pr_info("0x288B0004=0x%08X\n", readl(reg_base + 0x04));
	pr_info("0x288B0008=0x%08X\n", readl(reg_base + 0x08));
	pr_info("0x288B000C=0x%08X\n", readl(reg_base + 0x0C));
	pr_info("0x288B0010=0x%08X\n", readl(reg_base + 0x10));
	pr_info("0x288B0014=0x%08X\n", readl(reg_base + 0x14));
	pr_info("0x288B0018=0x%08X\n", readl(reg_base + 0x18));
	pr_info("0x288B001C=0x%08X\n", readl(reg_base + 0x1C));
	pr_info("0x288B0020=0x%08X\n", readl(reg_base + 0x20));
	pr_info("0x288B0024=0x%08X\n", readl(reg_base + 0x24));
	pr_info("0x288B0028=0x%08X\n", readl(reg_base + 0x28));
	pr_info("0x288B002C=0x%08X\n", readl(reg_base + 0x2C));
	pr_info("0x288B0030=0x%08X\n", readl(reg_base + 0x30));
	pr_info("0x288B0034=0x%08X\n", readl(reg_base + 0x34));
	pr_info("0x288B0038=0x%08X\n", readl(reg_base + 0x38));
	pr_info("0x288B003C=0x%08X\n", readl(reg_base + 0x3C));
	iounmap(reg_base);

}

static void dump_counter_reg(void)
{
	void __iomem *reg_base;

	reg_base = ioremap(0x2888c000, SZ_256);
	pr_info("0x2888C000=0x%08X\n", readl(reg_base + 0x00));
	pr_info("0x2888C004=0x%08X\n", readl(reg_base + 0x04));
	iounmap(reg_base);
}
...
struct audio_client *q6asm_audio_client_alloc(app_cb cb, void *priv)
{
	...
	if (ac->apr == NULL) {
		...
		if (my_flag == 0) {
			pr_info("The following register dumps will only be printed once per boot\n");
			dump_qdss_reg();
			dump_counter_reg();
			my_flag = 1;
		}
#endif
#ifdef Q6ASM_OPEN_FAIL_PANIC_TIMEOUT
		if(--open_fail_panic_timeout_cnt < 0) {
			panic("%s q6asm open failed\n", __func__);
		}
#endif
		goto fail;
	}
