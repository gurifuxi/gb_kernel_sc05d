 ...
 * but WITHOUT ANY WARRANTY; without even the implied warranty ofsdio_read_from_closed_ch
...
#define SDIO_BOOTLOADER_CARD_INDEX 1
 ...
 ||  defined (CONFIG_KOR_MODEL_SHV_E120K) || defined(CONFIG_KOR_MODEL_SHV_E160S) || defined(CONFIG_KOR_MODEL_SHV_E160K) || defined(CONFIG_KOR_MODEL_SHV_E160L) || defined (CONFIG_JPN_MODEL_SC_03D)  \
 || defined (CONFIG_USA_MODEL_SGH_I727) || defined (CONFIG_USA_MODEL_SGH_T989)|| defined (CONFIG_USA_MODEL_SGH_T769) || defined (CONFIG_USA_MODEL_SGH_I577) || defined (CONFIG_USA_MODEL_SGH_I717) \
...
#if defined (CONFIG_JPN_MODEL_SC_05D) //workaround for the case of read_avail > 0x4000
			sdio_read_from_closed_ch(ch, min(RX_FLUSH_BUFFER_SIZE, read_avail));
#else
...
static int sdio_ch_write(struct sdio_channel *ch, const u8 *buf, u32 len)
{
	...
	blksz = ch->func->cur_blksize;
	blocks = len / blksz;
	remain_bytes = len % blksz;
	fn = ch->func->num;
...
static void sdio_al_timer_handler(unsigned long data)
{
	...
	if (sdio_al_dev == NULL) {
		pr_err(MODULE_NAME ": %s: sdio_al_dev for data %lu \n", __func__,data);
		return;
	}
	if (sdio_al_dev->state != CARD_INSERTED) {
...
static void sdio_al_close_all_channels(struct sdio_al_device *sdio_al_dev)
{
	...
	if (!sdio_al_dev) {
		pr_err(MODULE_NAME ": %s: NULL device \n", __func__);
		return;
	}
	for (j = 0; j < SDIO_AL_MAX_CHANNELS; j++) {
