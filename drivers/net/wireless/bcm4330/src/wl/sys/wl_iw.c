...
#define WL_CMD(x)
#define WL_PRIVMSG(x)
// add WL_DBGMSG for personal log
#define WL_DBGMSG(x)
			...
			int param_type, void  *dst, int param_max_len)
{
	...
	if ((*str_ptr) && !strncmp(*str_ptr, token, strlen(token))) {
		...
		WL_DBGMSG((" 'token:%s', len:%d, ", token, parm_str_len));
		...
		*((char *)dst + parm_str_len) = 0; /* Z term */
		...
		WL_DBGMSG((" written as a string:%s\n", (char *)dst));
#endif
...
)
{
	...
	if (g_ssid.SSID_len) {
#ifdef BRCM_SECURITY_LOG		
		WL_DBGMSG(("%s: join SSID=%s BSSID="MACSTR" ch=%d\n", __FUNCTION__,
				...
				union iwreq_data *wrqu, char *ext)
{
	...
	extra[wrqu->data.length] = 0;
	WL_DBGMSG(("Got str param in iw_point:\n %s\n", extra));
...
set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	if (ap_cfg_running == FALSE) {
		...
		//dev_wlc_intvar_set(dev, "wme", 0); // wme enable
		...
		switch (param_type) {
			...
			/* string to decimal integer  */
				...
				*pdst_int = simple_strtoul(int_str, &eptr, 10);
				...
				WL_DBGMSG((" written as integer:%d\n",  *pdst_int));
			}
			break;
			case PTYPE_STR_HEX: {
				u8 *buf = dst;
...
#if ((defined(CONFIG_USA_MODEL_SGH_I717) || defined(CONFIG_USA_MODEL_SGH_T769) || defined(CONFIG_USA_MODEL_SGH_I577)) && defined(CHECK_CHIP_REV)) 
		if (g_chipver == 4) {
