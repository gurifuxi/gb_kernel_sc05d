...
#if defined(CONFIG_KOR_MODEL_SHV_E160S) || defined(CONFIG_KOR_MODEL_SHV_E160K) || defined (CONFIG_KOR_MODEL_SHV_E160L) || defined(CONFIG_JPN_MODEL_SC_05D) || defined(CONFIG_USA_MODEL_SGH_I717)
...
{
	int ret;
#if defined(CONFIG_JPN_MODEL_SC_05D)||defined(CONFIG_USA_MODEL_SGH_I717)||defined(CONFIG_USA_MODEL_SGH_T879)
	int retry = 5;	
#endif
		...
		if (object_type == PROCG_NOISESUPPRESSION_T48) {
			if (msg[4] == 5) { /* Median filter error */
					...
					if (ta_status) {
						...
						if (error)
							printk(KERN_ERR "[TSP] failed to T48 setting\n");
					}
					else 
					{
						...
						value = 45;
						write_mem(data, obj_address+7, 1, &value);

						value = 80;
						write_mem(data, obj_address+13, 1, &value);

						value = 3;
						write_mem(data, obj_address+22, 1, &value);
						value = 2;
						write_mem(data, obj_address+24, 1, &value);
			
						get_object_info(data, PROCG_NOISESUPPRESSION_T48, &size, &obj_address);
					     	value = 242;
					        write_mem(data, obj_address+2, 1, &value);
					        value = 20;
					        write_mem(data, obj_address+3, 1, &value);
					        value = 100;
					        write_mem(data, obj_address+17, 1, &value);
					        value = 25;
					        write_mem(data, obj_address+22, 1, &value);
					        value = 46;
					        write_mem(data, obj_address+25, 1, &value);
					        value = 112;
					        write_mem(data, obj_address+34, 1, &value);
					        value = 35; 
					        write_mem(data, obj_address+35, 1, &value);
					        value = 0;
					        write_mem(data, obj_address+39, 1, &value);
					        value = 40;
					        write_mem(data, obj_address+42, 1, &value);
						...
						get_object_info(data, SPT_GENERICDATA_T57, &size, &obj_address);
						value = 15;
						write_mem(data, obj_address+1, 1, &value);						
...
#if defined (CONFIG_KOR_MODEL_SHV_E160S) || defined (CONFIG_KOR_MODEL_SHV_E160K) || defined(CONFIG_KOR_MODEL_SHV_E160L) || defined(CONFIG_JPN_MODEL_SC_05D)// For Kor's Factory test Mode 
...
#if 0//defined(CONFIG_USA_MODEL_SGH_I717)
	if(retry <= 0)
		panic("[TSP] chip initialization failed_1\n");
#endif		
...
err_alloc_dev:
		...
		panic("[TSP] chip initialization failed_2\n");
	#endif
