...
static void pm8058_irq_bus_lock_without_mutex(unsigned int irq)
{
	u8	block;
	struct	pm8058_chip *chip = get_irq_data(irq);
...
static void pm8058_irq_bus_sync_unlock_without_mutex(unsigned int irq)
{
	u8	block, config;
	struct	pm8058_chip *chip = get_irq_data(irq);
...
#ifdef CONFIG_KOR_MODEL_SHV_E160S
	if (get_hw_rev() < 9) {
		chip->pm_max_masters = 4; // Workaround for spurious intr during boot
	}
	else
	{
	...
	for (i = 0; i < MAX_PM_IRQ; i++) {
		if (chip->config[i] && !chip->wake_enable[i]) {
			if (!((chip->config[i] & PM8058_IRQF_MASK_ALL)
				...
				pm8058_irq_bus_lock_without_mutex(irq);
				pm8058_irq_mask(irq);
				pm8058_irq_bus_sync_unlock_without_mutex(irq);
			}
		}
	}
