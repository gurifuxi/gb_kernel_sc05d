...
/* Function  : Change configuration descriptor
 * Parameter : int os type 
 *             1 - Mtp descriptor change for mac connection
 *             2 - Mtp descriptor change for windows connection
 * Description
 ...
 * Date : 2011-08-03
 */
static int mtp_set_config_desc(int os_type)
{
	switch (os_type) {
	case 1:
...
//			mtp_enable();
		...
		case MTP_WRITE_INT_DATA:
			printk("[%s] \t %d MTP intrpt_Write after msleep of 1 \n",__func__,__LINE__);
			msleep(1);
