		...
		enum usb_device_speed speed, void *buf, u8 type)
{
	...
	if (index_multi_conf > 0)
		printk("%s multi configuration: %d\n", __func__, index_multi_conf);
	...
	/*set interface number dynamically based on product function sequence. */
		...
		if (f->disabled || !descriptors || descriptors[0] == NULL)
			continue;
#ifdef CONFIG_USB_ANDROID_SAMSUNG_MTP
				/* Use vendor specific class for MAC OS */	
...
//				printk("Before MTP mode intf->bInterfaceClass: 0x%x\n", intf->bInterfaceClass);                             
				if(!strcmp(f->name, "mtp")) {
					if(config->cdev->MacPC == 1) {
...
static int config_desc(struct usb_composite_dev *cdev, unsigned w_value)
{
	...
	if (w_value != 0) {
		...
		printk("%s single configuration: %d\n", __func__, w_value);
	}
#endif  
...
static int count_configs(struct usb_composite_dev *cdev, unsigned type)
{
	...
	if (c->cdev == NULL) return count;
	if (c->cdev->products == NULL) return count;
 ...
 */
		...
		printk("mtp cmd id=0x%x)\n", id);
		for(i=0; i < cdev->product_num; i++) {
			char *product_function   = *product_functions++;
			if(!strcmp("mtp", product_function)) {
				printk("this product have mtp function (id=0x%x)\n", id);
				has_mtp = 1;
			}
		}

		if(has_mtp == 1) {
			os_string_descriptor_set *os_desc = buf;
			
			os_desc->bDescType = USB_DT_STRING;
			os_desc->bLength = 0x12;

			os_desc->qwsignature[0] = 0x4D;
			os_desc->qwsignature[1] = 0x00;
			os_desc->qwsignature[2] = 0x53;
			os_desc->qwsignature[3] = 0x00;
			os_desc->qwsignature[4] = 0x46;
			os_desc->qwsignature[5] = 0x00;
			os_desc->qwsignature[6] = 0x54;
			os_desc->qwsignature[7] = 0x00;
			os_desc->qwsignature[8] = 0x31;
			os_desc->qwsignature[9] = 0x00;
			os_desc->qwsignature[10] = 0x30;
			os_desc->qwsignature[11] = 0x00;
			os_desc->qwsignature[12] = 0x30;
			os_desc->qwsignature[13] = 0x00;

			os_desc->bMS_VendorCode = 0x54;
			os_desc->bPad = 0x00;
 ...
 */
		...
		if(cdev->products) {
			for(i = 0; i < cdev->product_num; i++) {
				if (!strcmp(cdev->products->functions[i], "mtp")) {
					...
					printk("%s 0x%x request, mode=mtp, p_num=%d, f_index=%d\n", 
							__func__, ctrl->bRequest, cdev->product_num, i);
					os_func_desc->bDescriptorType = 0x00;
					memcpy(os_func_desc->wData, &ms_descriptor, 38);
	...
	case ACCESSORY_START:
		...
		if (pusbfunction&& pusbfunction->setup) {  
			...
			if ( value< 0 ) {
				printk("composite_setup: accessory mode setup error \r\n");
			}
			else {
			...
			printk("composite_setup: usb function find fail \r\n");
		break;
#endif
#endif
