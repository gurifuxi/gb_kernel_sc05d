...
#define RCOMP0_TEMP 			20 	/* 'C */
#elif defined (CONFIG_KOR_MODEL_SHV_E160S) ||  defined (CONFIG_KOR_MODEL_SHV_E160L)
#if defined (CONFIG_PMIC8058_XOADC_CAL)
...
#define NB_HIGH_BLOCK_TEMP_ADC			450
#define NB_HIGH_RECOVER_TEMP_ADC		600
#define NB_EVT_HIGH_BLOCK_TEMP_ADC		220
#define NB_EVT_HIGH_RECOVER_TEMP_ADC		585
#define NB_LOW_BLOCK_TEMP_ADC			1570
#define NB_LOW_RECOVER_TEMP_ADC			1525
...
#define HIGH_BLOCK_TEMP_ADC_SETTHERM		384
#define HIGH_RECOVER_TEMP_ADC_SETTHERM	    357
#define LOW_BLOCK_TEMP_ADC_SETTHERM		    219
#define LOW_RECOVER_TEMP_ADC_SETTHERM	    234
#elif defined (CONFIG_KOR_MODEL_SHV_E160K)
...
#define RE_CHG_COND_COUNT		4 //8
#define RE_CHG_MIN_COUNT		2 //4
...
#elif defined  (CONFIG_USA_MODEL_SGH_I717)
	if((get_hw_rev() == 0x01) && (adc_physical > 1290 && adc_physical < 1800))
		value.intval = BAT_DETECTED;
	else if((get_hw_rev() != 0x01) && (adc_physical > 300 && adc_physical < 660)) 
#endif
		value.intval = BAT_DETECTED;
	else
...
#endif /* CONFIG_PMIC8058_XOADC_CAL */
#elif defined (CONFIG_USA_MODEL_SGH_I727) || defined (CONFIG_USA_MODEL_SGH_I717) || defined(CONFIG_USA_MODEL_SGH_I757) || defined (CONFIG_USA_MODEL_SGH_I577)
#if defined (CONFIG_PMIC8058_XOADC_CAL)
const int temper_table[][2] =  {
	/* ADC, Temperature (C) */
	{ 1755,		-100	}, 				
...
static int sec_bat_check_temper_adc(struct sec_bat_info *info)
{
	...
	*/
	...
	if (rescale_adc >= JPN_CHARGE_CURRENT_DOWN_TEMP) {
		if (info->jpn_chg_cur_ctrl_status != POWER_SUPPLY_HEALTH_OVERHEAT)
	...
	//pr_info("[SC-03D] %s : adc=%d, jpn_chg_cur_ctrl_status=%d, jpn_temp_high_cnt=%d\n", __func__,
	//		rescale_adc, info->jpn_chg_cur_ctrl_status, info->jpn_chg_cur_ctrl_temp_high_cnt);
#endif
	...
	*/
	...
	if (!info->lpm_chg_mode &&
		...
		if (set_chg_current >= 0) {
			if (info->jpn_chg_cur_ctrl_status == POWER_SUPPLY_HEALTH_GOOD) {
				if (info->is_esus_state) {
					if (info->voice_call_state == 0 &&
						set_chg_current != 1000) {
						printk("[SC-03D] %s : adjust current to 1A\n", __func__);
						sec_bat_adjust_charging_current(info, 1000);
					}
				} else {
					if (set_chg_current != 900) {
						printk("[SC-03D] %s : adjust current to 0.9A\n", __func__);
				...
				if (set_chg_current != 600) {
					printk("[SC-03D] %s : Temp is above 41celsius, adjust current to 600mA, temp adc=%d\n", __func__, info->batt_temp_radc);
					sec_bat_adjust_charging_current(info, 600);
				}
			}
		} else {
