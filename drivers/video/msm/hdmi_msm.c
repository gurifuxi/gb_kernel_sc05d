...
struct hdmi_msm_state_type {
	...
	boolean resolution_switch_signal;
...
static void hdmi_msm_hpd_state_work(struct work_struct *work)
{
	...
	if (!hdmi_msm_state->hpd_cable_chg_detected) {
		...
		if (hpd_state) {
			...
			wait_event_interruptible_timeout(hdmi_msm_resolution_switchEvent, hdmi_msm_state->resolution_switch_signal, msecs_to_jiffies(1*1000)); // Placed here instead of hdcp enable function as this is a first time issue and when the auth_fail_int happens the hdcp_enable function should not wait unnecessarily.
			hdmi_msm_state->resolution_switch_signal = FALSE;// set to false for the next sync detect.
...
static int hdcp_authentication_part1(void)
{
	 ...
	 * during reauthentication in MSM8960 & APQ8060
	 */
	msleep(20);
...
//Added to fix boot up HDMI issue
...
#ifdef MHL_HPD_SOLUTION
	/* Celox series : Fix the sleep current issue.(mhl_v2)*/
	...
	if (var->reserved[3]) {
			...
			hdmi_msm_state->resolution_switch_signal = TRUE;
...
#if 0 /*When change the resolution, reset issue.*/
	mutex_lock(&hdmi_msm_state_mutex);
	if (hdmi_msm_state->hdcp_activating) {
...
#if !defined(CONFIG_VIDEO_MHL_V1) && !defined(CONFIG_VIDEO_MHL_V2) // Hpd is not turned on or off baseed on the user option. Mhl triggers the on and dtv_off puts the hdmi in off state..
	mutex_lock(&external_common_state_hpd_mutex);
	if (!external_common_state->hpd_feature_on)
		hdmi_msm_hpd_off();
	mutex_unlock(&external_common_state_hpd_mutex);
...
static int __devinit hdmi_msm_probe(struct platform_device *pdev)
{
	...
	hdmi_msm_state->resolution_switch_signal = FALSE;
...
static int hdmi_msm_device_pm_suspend(struct device *dev)
{
	...
	if (!hdmi_msm_state || !HDMI_BASE || !hdmi_msm_state->mhl_hpd_state) {  //Praveen: probe failed hence no use in disabling/enabling clks as they are released.(  if probe failed, SS has seen kernel panic at this point as clks are released and set to NULL, enable/disable call caused panic).
		DEV_DBG("pm_suspend returns 0 - HDMI_BASE(0x%x), mhl_hpd_state(%d)\n", HDMI_BASE, hdmi_msm_state->mhl_hpd_state);
		return 0;
		}
...
{
	if (!hdmi_msm_state || !HDMI_BASE || !hdmi_msm_state->mhl_hpd_state) {  //Praveen: probe failed hence no use in disabling/enabling clks as they are released.(  if probe failed, SS has seen kernel panic at this point as clks are released and set to NULL, enable/disable call caused panic).
		DEV_DBG("pm_resume returns 0 - HDMI_BASE(0x%x), mhl_hpd_state(%d)\n", HDMI_BASE, hdmi_msm_state->mhl_hpd_state);
		return 0;
		}
