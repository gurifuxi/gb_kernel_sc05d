...
int mdp4_dsi_video_on(struct platform_device *pdev)
{
	...
	if (dsi_pipe == NULL) {
		...
		if (ptype < 0)
			printk(KERN_INFO "[MDP4] %s: format2type failed\n", __func__);
		pipe = mdp4_overlay_pipe_alloc(ptype, MDP4_MIXER0, 0);
		if (pipe == NULL) {
			printk(KERN_INFO "[MDP4] %s: pipe_alloc failed\n", __func__);
			return -EBUSY;
		}
		pipe->pipe_used++;
...
/* 3D side by side */
void mdp4_dsi_video_3d_sbys(struct msm_fb_data_type *mfd, struct msmfb_overlay_3d *r3d)
{
	struct fb_info *fbi;
	struct mdp4_overlay_pipe *pipe;
	int bpp;
 ...
 */
static void mdp4_overlay_dsi_video_wait4event(struct msm_fb_data_type *mfd,	int intr_done)
{
	unsigned long flag;
	...
	switch(update_type)
	{
		...
		case NO_WAIT_SYNC_CALL:
			...
			if( (tv_from_wait.tv_sec == tv_from_no_wait.tv_sec) && ((tv_from_no_wait.tv_usec - tv_from_wait.tv_usec) < LEAST_DELAY_TIME)  )
			{
				...
				printk("[MDP4] == Too short time to update 1 - current latency time %d, delay %d\n",latency_time, LEAST_DELAY_TIME - latency_time);
			...
			else if( (tv_from_wait.tv_sec != tv_from_no_wait.tv_sec) && (tv_from_no_wait.tv_usec < tv_from_wait.tv_usec) )
			{
				...
				{
					printk("[MDP4] == Too short time to update 2- current latency time %d, delay %d\n",latency_time, LEAST_DELAY_TIME - latency_time);
					usleep(LEAST_DELAY_TIME - latency_time); 
				}
			}
...
void mdp4_overlay_dsi_video_vsync_push(struct msm_fb_data_type *mfd, struct mdp4_overlay_pipe *pipe)
{
	unsigned long flag;
...
void mdp4_overlay0_done_dsi_video(struct mdp_dma_data *dma)
{
	...
	dma->busy = FALSE;
	if (dsi_pipe->blt_addr == 0) { // CRs-fixed: 310918
