...
static int m5mo_mipi_mode(int mode)
{
	...
	if (!config_csi) {
		/* MIPI stop state (LP11) : monitor -> parm -> LP11 state */
		CAM_DEBUG("set M5MO_MONITOR_MODE");
		...
		CAM_DEBUG("set M5MO_PARMSET_MODE");
		m5mo_set_mode(M5MO_PARMSET_MODE);

		/* MIPI CSI config at LP11 */
		CAM_DEBUG("start CSI configuration");
		...
		m5mo_csi_params.settle_cnt = 0x2F; // 24 -> 20 -> 47
...
#if 1 // to check mipi interrupt status
extern void __iomem *csibase;
...
static long m5mo_set_sensor_mode(int mode)
{
	...
	case SENSOR_PREVIEW_MODE:
		...
		m5mo_mipi_mode(mode);
		m5mo_preview_mode();
#if 1 // to check mipi overflow issue
		{
			uint32_t irq; 
