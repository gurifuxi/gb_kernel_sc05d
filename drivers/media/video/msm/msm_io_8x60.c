...
uint32_t save_mipi_irq; // to check mipi interrupt status
static u8 check_first_mipioverflow;
...
{
	uint32_t irq;
	uint32_t mipidata_overflow;
	
	irq = msm_io_r(csibase + MIPI_INTERRUPT_STATUS);
	save_mipi_irq = irq;
	mipidata_overflow = 0; 
	mipidata_overflow = irq & 0x30000000; // DL0, DL1 overflow
	if(mipidata_overflow) 
		check_first_mipioverflow = 1;

	if(check_first_mipioverflow)
	{
		printk("%s ### First MIPI_INTERRUPT_STATUS = 0x%x\n", __func__, irq);
		check_first_mipioverflow = 0;
	}		
